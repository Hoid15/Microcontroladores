...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
#define BOTON_ON GPIO_PIN_0;
#define BOTON_OFF GPIO_PIN_1;
...
int Func_ESTADO_INTERMEDIO(void){
	...
	for (;;) {
		...
		if ((inout.Sa == TRUE)&&(ESTADO_ANTERIOR == ESTADO_INICIAL)) {
...
int Func_ESTADO_MEDIDOR(void){
	...
	for (;;) {
		...
		lcd_clear();
		lcd_enviar("Voltaje:", 0, 1);
		lcd_enviar(VOL_STAR, 1, 2);
		HAL_Delay(1000);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim2){
	...
	if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_0) == TRUE) {
	...
	static uint32_t MED_ADC = 0;



	if (ESTADO_Medidor == ESTADO_ON) {
